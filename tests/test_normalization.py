...
from stock_data.normalization import CANONICAL_COLUMNS, NormalizationError, normalize_symbol


def yahoo_frame() -> pd.DataFrame:
    return pd.DataFrame(
        {"Open": [100.0, 101.0], "High": [105.0, 106.0], "Low": [99.0, 100.0],
         "Close": [104.0, 105.0], "Volume": [1000, 1100]},
...
def test_normalize_symbol_rejects_missing_required_column() -> None:
    with pytest.raises(NormalizationError, match="missing columns"):
        normalize_symbol("TCS.NS", yahoo_frame().drop(columns="Volume"), date(2026, 6, 5))
