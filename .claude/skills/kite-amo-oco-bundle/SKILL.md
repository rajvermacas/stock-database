...
description: Place the user's standard swing-trade order bundle through the Kite MCP — an AMO market entry plus two OCO GTT protective exits (−3% stop, +3% and +15% targets, 50/50 split, 0.5% limit buffer). Use when the user asks to buy/enter one or more stocks, "set up <SYMBOL>", "place my usual order for X", or wants the full entry+exit bracket on a name.
---

# Kite AMO + Dual-OCO Bundle

Place the user's default swing-trade structure for one or more stocks via the Kite
MCP tools: a **delivery AMO market buy**, protected by **two OCO (two-leg) GTT
sell orders** that split the position 50/50 — both with a −3% stop, one targeting
+3% and the other +15%. All GTT legs are LIMIT orders with a 0.5% buffer past the
trigger to fill like a market order (Kite has no market GTT).
...
| Targets          | +3% (first GTT) and +15% (second GTT) |
...
5. **Compute levels** from the LTP basis, then round each to the ₹0.05 tick:
   - Stop trigger  = basis × 0.97
   - Target A trig = basis × 1.03
   - Target B trig = basis × 1.15
   - Each leg's **limit** = its trigger × 0.995 (0.5% below), re-rounded to tick.
   - Sanity check: the stop trigger must be **below** the LTP and both target
     triggers **above** it, or the OCO GTT is invalid at placement. If the LTP has
...
6. **Show the review table** — one combined table when batching — listing for each
   symbol: exchange, qty, LTP basis, the AMO line, and both GTTs (trigger + buffered
   limit + qty per leg). State that levels are LTP estimates and the real −3/+3/+15
   shift slightly if the AMO fills at a different price. Then wait for `confirm`.
...
7. **On confirm, place in order** per symbol:
   ...
   - **GTT A (+3%)** — `mcp__kite__place_gtt_order`:
   ...
   - **GTT B (+15%)** — same as A but `upper_trigger_value=<t15>` and its buffered
     limit; same stop and `lower_quantity` / `upper_quantity = qty/2`.
...
| GTT A (10) | stop 492.30→489.85 · target +3% 522.75→520.15 |
| GTT B (10) | stop 492.30→489.85 · target +15% 583.65→580.75 |

## Notes & guardrails

- These are real, irreversible orders. Always review-then-confirm; never skip step 6.
- The two GTTs are **separate** OCO orders on the same instrument — Kite allows this.
