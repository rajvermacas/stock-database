...
class PriceStore:
    ...
    def path_for(self, symbol: str) -> Path:
        ...
        return self.prices_dir / f"{symbol}.parquet"
    ...
    def latest_date(self, symbol: str) -> date | None:
        ...
        return None if frame is None else frame["trade_date"].max()
    ...
    def upsert(self, symbol: str, new_rows: pl.DataFrame) -> WriteResult:
        ...
        merged = merged.unique(["symbol", "trade_date"], keep="last").sort("trade_date")
    ...
    def write_atomic(self, symbol: str, frame: pl.DataFrame) -> None:
        ...
        try:
            with tempfile.NamedTemporaryFile(dir=self.prices_dir, delete=False) as file:
